...
public class FlexBuffersBuilder {
    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        for (int i = start; i < stack.size(); i++) {
            ...
            if (typed) {
                if (i == start) {
                    ...
                    if (!isTypedVectorElementType(vectorType)) {
        ...
        // Int / UInt / Float / Key.
        assert (!fixed || isTypedVectorElementType(vectorType));
        ...
        return new Value(key, keys != null ? FBT_MAP
                : (typed ? toTypedVector(vectorType, fixed ? length : 0)
    ...
    private Value createKeyVector(int start, int length) {
        ...
        // Then the types.
        return new Value(-1, toTypedVector(FBT_KEY,0), bitWidth, vloc);
    }
